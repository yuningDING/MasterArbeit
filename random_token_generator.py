...
def _get_token_ngrams(token_list, n):
    ...
    return token_ngrams, ngram_set, ngram_frequency


def _get_ngram_frequency(token_ngrams):
    ngram_frequency = {}
    ngram_char_set = {}
    for ngram in token_ngrams.keys():
        char_list = token_ngrams.get(ngram)
        char_list.sort()
...
def get_text(ngram_set, ngram_frequency, ngram_dict, ngram_token_set, ngram_frequency_dict, n=3, sentence_length=140,
             start=None):
    ...
    while current_length < sentence_length:
        ...
        if n > 1:
            ...
            next_gram = next_gram = choice(a=ngram_set, size=1, p=ngram_frequency)[0]
...
if __name__ == "__main__":
    ...
    for i in range(1, 11):
        n = 1
        with open('outputs/token_1-5gram/token_' + str(n) + '_gram_prompt_' + str(i) + '_1000.txt', 'w',
            ...
            ngram_dict, ngram_set, ngram_frequency = _get_token_ngrams(token_list, n)
            ...
            for j in range(1000):
                sentence = get_text(ngram_set, ngram_frequency, ngram_dict, ngram_token_set, ngram_frequency_dict, n,
                                    max_length[str(i)])
                while len(sentence) < 3:
                    sentence = get_text(ngram_set, ngram_frequency, ngram_dict, ngram_token_set, ngram_frequency_dict,
                                        n, max_length[str(i)])
